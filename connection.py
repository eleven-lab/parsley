...
def _Recv ( s ):
	...
	try:
		# i cant wait to receive all data otherwise other endpoint peer may close connection for timeout
		...
		if ( len( data ) == 0 ): #FIN by endpoint peer --> When a stream socket peer has performed an orderly shutdown, 
			...
			#fin = 2 # close the connection
			return ( 2, data ) # empty
		elif ( len( data ) < BUF_SIZE ): #FIN by endpoint peer --> not true, only finished to send data this does not mean tha it wants to close connection
			
			#fin = 1 # finished to send data --> u sure about that? no
			return ( 1, data )
		else:
			#there is more data to be received, max socket space occupied
			return ( 0, data )
			
	except ssl.SSLError:
		#logging.error ( "[!] ssl.SSLError on RECEIVING data!", exc_info=True )
		return (0,-1)	
		...
		# SSLWantReadError will be raised if a read operation on the underlying socket is necessary, and SSLWantWriteError for a write operation on the underlying socket. Note that attempts to write to an SSL socket may require reading from the underlying socket first, and attempts to read from the SSL socket may require a prior write to the underlying socket.
		#logging.error ( "[!] socket.error on RECEIVING data!", exc_info=True )
		return (0,-1)
...
def handle_connection ( client, server ):
	...
	client.settimeout ( 0 ) # set blocking socket client
	...
	while True:
		...
		client_fin, client_data = _Recv ( client )
		
		# what if client has more data to send? incomplete data
		...
		if ( client_fin==1 ): # presumably finished of sending request
			...
			#server.settimeout ( None ) # set server to blocking
			server.settimeout ( 0 )
		...
		if ( client_data != -1 ):
			...
			_Sendall( server, client_data ) # this will send ALSO FIN by client
		...
		server_fin, server_data = _Recv( server )
		# what if listening on a closed connection? what if server is still waiting data from client?
		...
		if ( server_data != -1 ): 
			...
			_Sendall( client, server_data ) # server didn't sent anything

		if ( server_fin==1 ):
			# server finish to send data only when it wants to close connection
